# -*- coding: utf-8 -*-
"""
Created on Tue Mar 16 19:10:59 2021
...
parser.add_argument('--dataset', default='douban', help='dataset name: Ciao/Epinions')
...
# =============================================================================
workdir = 'G:/recommender/pytorch/owncode/nanshou/datasets_pre/'
...
# 	trust_f = loadmat(workdir + 'Epinions/trustnetwork.mat')['trustnetwork']
    ...
    click_f = np.loadtxt(workdir+'toy/rating.txt', dtype = np.int32)
    trust_f = np.loadtxt(workdir+'toy/trust.txt', dtype = np.int32)
    train_f = np.loadtxt(workdir+'toy/train.txt', dtype = np.int32)
    test_f = np.loadtxt(workdir+'toy/test.txt', dtype = np.int32)
...
trust_df = pd.DataFrame(train_f, columns = ['uid', 'iid','label'])
